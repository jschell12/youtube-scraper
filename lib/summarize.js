...
// Max words per LLM call — sized to fit the local Ollama model's context.
 ...
 */
export async function summarizeVideo(filePath, { model = 'ollama' } = {}) {
  ...
  // Short transcripts fit in one pass. Long ones (podcasts, livestreams) get
  // map-reduce: summarize each chunk into compact notes, then apply the
  // template to the combined notes so the whole video is covered — previously
  // only the first 3000 words were summarized and the rest was dropped.
  let sourceText;
  if (words.length <= CHUNK_WORDS) {
    ...
    console.log(`  Summarizing (${label}, ${chunks.length} chunks): ${title}`);
    const notes = [];
    for (let i = 0; i < chunks.length; i++) {
      const notePrompt = `You are condensing part ${i + 1} of ${chunks.length} of a video transcript titled "${title}". Write dense factual notes (bullet points) covering every distinct topic, claim, name, number, and recommendation in this section. Correct obvious speech-to-text errors in names when the intended person or product is clear. Do not editorialize or omit sections.

Transcript section:
${chunks[i]}`;
      notes.push(await callClaude(notePrompt, { model, timeout: 120_000 }));
    }
    sourceText = `(Notes condensed from the full ${chunks.length}-part transcript)\n\n${notes.map((n, i) => `--- Part ${i + 1} ---\n${n}`).join('\n\n')}`;
  }
 ...
 * @param {string} opts.model
 */
export async function summarizeDay(dayDir, { model = 'ollama' } = {}) {
