...
/**
 * Every channel discovery should not propose again: ids already in the store,
 * plus the channels already being scraped.
 *
 * Sources identify channels two ways — /channel/UC… URLs give an id outright,
 * @handle URLs do not, so handles come back separately and are matched against
 * each candidate's snippet.customUrl once enrichment has fetched it.
 */
export function knownChannels(dataDir, sources = []) {
  const ids = new Set(Object.keys(readSuggestions(dataDir).channels));
